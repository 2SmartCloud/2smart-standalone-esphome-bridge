...
config
{
    ...
    state_off - string (optional, default: OFF)
    state_on - string (optional, default: ON)
    state_topic - string (optional)
    unique_id - string (optional)
    value_template - template (optional)
}
*/
class HomeAssistantSwitchComponent extends HomeAssistantComponent {
    constructor(config, { mqttConnection, allStates, debug, node_id, object_id }) {
        ...
        _.defaults(config, {
            state_off : config.payload_off,
            state_on  : config.payload_on
        });
        super(config, { mqttConnection, allStates, debug, domain: 'switch', node_id, object_id });
    ...
    init() {
        ...
        if (this.config.state_topic) {
            ...
            const state_on = this.config.state_on;
            const state_off = this.config.state_off;
            const handler = (value) => {
                try {
                    let value_json = null;
                    ...
                    if (`${result}` === `${state_on}`) {
                        this.stateOn = true;
                    } else if (`${result}` === `${state_off}`) {
                        this.stateOn = false;
                    } else {
                        throw new Error(`Received wrong switch value ${result}`);
                    }
...
module.exports = HomeAssistantSwitchComponent;
