...
function _apply_abbreviations(config, abbrs) {
    for (const k in config) {
        if ((k in abbrs) && abbrs[k] !== k) {
            // eslint-disable-next-line no-param-reassign
            config[abbrs[k]] = config[k];
            // eslint-disable-next-line no-param-reassign
            delete config[k];
        }
    }
...
function apply({ ...config }) {
    _apply_abbreviations(config, abbreviations.general);

    if (config.device) _apply_abbreviations(config.device, abbreviations.device);
